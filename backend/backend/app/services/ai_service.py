...
location/time resolution, existing Phase 2-4 services, and Groq together.
...
async def handle_chat(message: str, current_lat: float, current_lon: float) -> ChatResponse:
    ...
    try:
        parsed = await parse_query(message, settings.groq_api_key, settings.groq_model)
    except QueryUnderstandingError as exc:
        logger.warning("Groq query understanding failed, falling back to UNKNOWN: %s", exc)
        return ChatResponse(
            answer=(
    ...
    # --- 3. Time resolution (deterministic, not the AI) ---
    ...
    ai_context = {
        ...
        "official_alerts": trimmed_alerts_for_ai,
        "weathergpt_risk_assessment": risk_result,
    }

    # --- 7. Groq grounded answer ---
    ai_status = "available"
    try:
        answer = await generate_answer(
            message, ai_context, settings.groq_api_key, settings.groq_model
        )
    except ResponseGenerationError as exc:
        logger.warning("Groq response generation failed, degrading gracefully: %s", exc)
        ai_status = "degraded"
        answer = (
